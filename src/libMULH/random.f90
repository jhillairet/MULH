...
function poisdev(lambda, atype) result(r)
!**********************************************************************
! Poisson RNG based on waiting time method from MATLAB
!
! mu = mean
!
! right now it only works for integer lambda.
!
use precision_def
use ecuyer_taus_rng
implicit none
! Function Arguments
real(long), intent(in) :: lambda
...
if (lambda < 0) then
  write(*,*) 'Error in poisdev. Means cant be negative. Terminating'
  call exit
endif
...
if (lambda > 1e20) then
  write(*,*) 'Error in poisdev. Infinite mean. Terminating'
  call exit
endif

if (isNaN(lambda)) then
  write(*,*) 'Error in poisdev. NaN mean. Terminating'
  call exit
endif
