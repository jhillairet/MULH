...
project_path = '/Home/AP252436/Work_MULH_essais/Parallel_batch/'
# config files
config_file = 'config.mulh'

B_xs = np.append(0, np.logspace(-3, 0, num=41))
...
#    if power: 
#        power_results, time_results = mulh.get_full_results()         # get_full_results à coder
#        multipactor_order = power_results.dropna()['Multipactor order'].iat[-1]
#        power_results = 42
#        time_results = 42
#        multipactor_order = 42
#    else:
#        power = np.NaN        
#        multipactor_order = np.NaN       
#        print('ligne 55')
    
    # Write results
    with open('/Home/AP252436/Work_MULH_essais/Parallel_batch/Pth_vs_Bx_C3_SEY-MULH_100electrons.csv','ba') as f_handle:   
